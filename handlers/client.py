...
async def available_commands(message: types.Message):
    await bot.send_message(chat_id=message.chat.id, text="""<b>add N - основная команда для изменение счёта,</b> 
...
def registe_handlers_client(dp: Dispatcher):
    ...
    dp.register_message_handler(about_creators, lambda message: message.text.startswith('О создателе 👨‍💻'))
